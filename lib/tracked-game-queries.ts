...
export async function getTrackedPlayoffWeek(season: number) {
  const rows = await asPublic<{ week: number | null }>(
    `select max(m.week)::int as week from public.matchups m
      where m.season=$1 and m.is_final and m.playoff_tier=$2`,
  ...
  const games = await asPublic<{
    ...
    `select m.espn_matchup_id,m.playoff_tier,
      ...
      where m.season=$1 and m.week=$2 and m.playoff_tier=$3 order by m.espn_matchup_id`,
...
export async function getTrackedTopScoringWeeks(limit = 10) {
  ...
  return asPublic<{
    ...
    `with sides as (
       select m.season,m.week,m.playoff_tier,m.home_team_id as espn_team_id,
              m.home_points as points_for,m.away_team_id as opponent_team_id,m.away_points as points_against,
              case m.winner when 'HOME' then 'W' when 'AWAY' then 'L' when 'TIE' then 'T' end as result
         from public.matchups m where m.is_final and m.home_points is not null and ${tracked}
       union all
       select m.season,m.week,m.playoff_tier,m.away_team_id,m.away_points,m.home_team_id,m.home_points,
...
export async function getTrackedTopPlayerWeeks(limit = 10) {
  ...
  return asPublic<{
    ...
    `select r.season,r.week,pi.player_key,r.espn_player_id,pi.full_name,
      ...
      where r.applied_points is not null order by r.applied_points desc limit $1`, [limit]
  );
}

type TrackedMatchupRecordKind = 'highest_score'|'lowest_score'|'highest_scoring_loss'|
  'lowest_scoring_win'|'highest_combined_score'|'biggest_blowout'|'closest_finish';

async function getTrackedMatchupRecord(season: number | null, kind: TrackedMatchupRecordKind) {
  const tracked = trackedMatchupSql('m');
  const filter = kind === 'highest_scoring_loss' ? `and x.result='L'`
    : kind === 'lowest_scoring_win' || kind === 'biggest_blowout' ? `and x.result='W'` : '';
  const order = kind === 'highest_score' || kind === 'highest_scoring_loss' ? 'x.points_for desc'
    : kind === 'lowest_score' || kind === 'lowest_scoring_win' ? 'x.points_for asc'
    : kind === 'highest_combined_score' ? '(x.points_for+x.points_against) desc, greatest(x.points_for,x.points_against) desc'
    : kind === 'biggest_blowout' ? 'abs(x.points_for-x.points_against) desc'
    : 'abs(x.points_for-x.points_against) asc, x.points_for desc';
  const rows = await asPublic<MatchupRecordRow>(
    `with sides as (
       select m.season,m.week,m.playoff_tier,m.home_team_id as espn_team_id,m.away_team_id as opponent_team_id,
         ...
         from public.matchups m where m.is_final and m.home_points is not null and m.away_points is not null and ${tracked}
       union all
       select m.season,m.week,m.playoff_tier,m.away_team_id,m.home_team_id,m.away_points,m.home_points,
              case m.winner when 'AWAY' then 'W' when 'HOME' then 'L' else 'T' end
         from public.matchups m where m.is_final and m.home_points is not null and m.away_points is not null and ${tracked}
     )
     select x.season,x.week,x.espn_team_id,fs.franchise_key,fs.team_name,ofs.team_name as opponent_name,
            round(x.points_for,1)::text as points_for,round(x.points_against,1)::text as points_against,
            x.result,round(x.points_for-x.points_against,1)::text as margin,nullif(x.playoff_tier,'NONE') as playoff_tier
       from sides x
       join public.franchise_season_teams fs on fs.season=x.season and fs.espn_team_id=x.espn_team_id
       join public.franchise_season_teams ofs on ofs.season=x.season and ofs.espn_team_id=x.opponent_team_id
      where ($1::int is null or x.season=$1) ${filter} order by ${order} limit 1`, [season]
  );
  return rows[0] ?? null;
}
...
export async function getTrackedTopPlayerWeekForSeason(season: number) {
  ...
  const rows = await asPublic<PlayerWeekRecordRow>(
    `select r.season,r.week,r.espn_player_id,pi.full_name,pi.position_id as default_position_id,
            round(r.applied_points,1)::text as points,fs.franchise_key,fs.team_name,r.is_starter,
            nullif(m.playoff_tier,'NONE') as playoff_tier
       from public.roster_entries r
       join public.player_identity pi on pi.season=r.season and pi.espn_player_id=r.espn_player_id
       join public.franchise_season_teams fs on fs.season=r.season and fs.espn_team_id=r.espn_team_id
       join public.weeks w on w.season=r.season and w.week=r.week and w.results_complete
       join public.matchups m on m.season=r.season and m.week=r.week
        and r.espn_team_id in(m.home_team_id,m.away_team_id) and ${tracked}
      where r.season=$1 and r.applied_points is not null order by r.applied_points desc limit 1`, [season]
  );
  return rows[0] ?? null;
}
...
export async function getTrackedSeasonHighlights(season: number) {
  const [highestScore,biggestBlowout,closestFinish,topPlayer] = await Promise.all([
    getTrackedMatchupRecord(season,'highest_score'),getTrackedMatchupRecord(season,'biggest_blowout'),
    getTrackedMatchupRecord(season,'closest_finish'),getTrackedTopPlayerWeekForSeason(season),
  ]);
  return { highestScore,biggestBlowout,closestFinish,topPlayer };
}

export async function getTrackedGameRecords() {
  const [highestScore,lowestScore,highestScoringLoss,lowestScoringWin,highestCombinedScore,biggestBlowout,closestFinish] = await Promise.all([
    getTrackedMatchupRecord(null,'highest_score'),getTrackedMatchupRecord(null,'lowest_score'),
    getTrackedMatchupRecord(null,'highest_scoring_loss'),getTrackedMatchupRecord(null,'lowest_scoring_win'),
    getTrackedMatchupRecord(null,'highest_combined_score'),getTrackedMatchupRecord(null,'biggest_blowout'),
    getTrackedMatchupRecord(null,'closest_finish'),
  ]);
  return { highestScore,lowestScore,highestScoringLoss,lowestScoringWin,highestCombinedScore,biggestBlowout,closestFinish };
}
