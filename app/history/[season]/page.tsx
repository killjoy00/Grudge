...
import { getCachedPowerRankings, getCachedSeasonList } from '../../../lib/cached-queries.ts';
...
export default async function SeasonHistoryPage({ params }: { params: Promise<{ season: string }> }) {
  ...
  const [[rows, managers, playoffGames, highlights], seasonList, powerRows] = await Promise.all([
    getCachedHistorySeason(season),
    getCachedSeasonList(),
    getCachedPowerRankings(season),
  ]);
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', margin: '0 0 24px' }}>
        ...
        <a className="btn btn-quiet" href={`/standings?season=${season}`}>Standings</a>
        <a className="btn btn-quiet" href={`/rankings?season=${season}`}>Power rankings</a>
        <a className="btn btn-quiet" href={`/history/vault/${season}`}>Vault evidence</a>
      ...
      {champion && (
        <>
          ...
          <div className="card">
            ...
            {sameChampion && <p className="note">They also finished first in the regular-season standings.</p>}
            {managerByFranchise.get(champion.franchise_key) && (
              <p className="note" style={{ marginBottom: 0 }}>
                Managed by <a href={managerHref(managerByFranchise.get(champion.franchise_key)!.manager_key)}>{managerByFranchise.get(champion.franchise_key)!.display_name}</a>.
              </p>
            )}
          </div>
        </>
              ...
              {highlights.topPlayer && (
                <div>
                  ...
                  <span className="block">
                    {highlights.topPlayer.full_name ?? 'Unknown player'} — {highlights.topPlayer.points} for <a href={franchiseHref(highlights.topPlayer.franchise_key)}>{highlights.topPlayer.team_name}</a>, week {highlights.topPlayer.week}
                    {!highlights.topPlayer.is_starter && ' (benched)'}
                  </span>
                </div>
              )}
              {highlights.biggestBlowout && <div><strong>Biggest blowout</strong><span className="block">{gameSummary(highlights.biggestBlowout)} · {Number(highlights.biggestBlowout.margin).toFixed(1)} points</span></div>}
              {highlights.closestFinish && <div><strong>Closest finish</strong><span className="block">{gameSummary(highlights.closestFinish)} · {Math.abs(Number(highlights.closestFinish.margin)).toFixed(1)} points</span></div>}
            </div>
          </div>
        </>
      )}
      ...
      {playoffGames.length > 0 && (
        <>
          ...
          <div className="card"><div className="scroll"><table>
            ...
            <tbody>{playoffGames.map((game) => {
              const homeWon = game.winner === 'HOME';
              const winner = homeWon ? { key: game.home_key, name: game.home_name, points: game.home_points } : { key: game.away_key, name: game.away_name, points: game.away_points };
              const loser = homeWon ? { key: game.away_key, name: game.away_name, points: game.away_points } : { key: game.home_key, name: game.home_name, points: game.home_points };
              return (
