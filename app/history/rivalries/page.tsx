        ...
        <div style={{ display: 'grid', gap: 20 }}>
          {highlights.mostPlayed && (
            <div>
              ...
              <span className="block note">{highlights.mostPlayed.games} games · {record(highlights.mostPlayed)} from {highlights.mostPlayed.team_a_name}'s side · since {highlights.mostPlayed.first_season}</span>
            </div>
          )}
          ...
          {highlights.playoffNemesis && playoffNemesis && (
            <div>
              ...
              <span className="block note">
                <a href={hrefFor(highlights.playoffNemesis)}>
                  {playoffNemesis.wins}-{playoffNemesis.losses} in the playoffs against {playoffNemesis.id === highlights.playoffNemesis.team_a_id ? highlights.playoffNemesis.team_b_name : highlights.playoffNemesis.team_a_name} · {highlights.playoffNemesis.playoff_games} playoff meetings →
                </a>
              </span>
            </div>
          )}
          ...
          {highestGame && (
            <div>
              ...
              <span className="block note">
                <a href={`/rivalry/${highestGame.home_team_id}/${highestGame.away_team_id}`}>{highestGame.total_points} combined points · {highestGame.season} week {highestGame.week}{highestGame.playoff_tier ? ' · playoffs' : ''} →</a>
              </span>
            </div>
          )}
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th className="num">Playoff record</th>
            ...
            <tbody>
              {pairs.map((row) => (
                <tr key={`${row.team_a_id}-${row.team_b_id}`}>
                  ...
                  <td className="num">{row.playoff_games || '—'}</td>
                  <td className="num">{row.playoff_games ? playoffRecord(row) : '—'}</td>
                  <td className="num">{row.first_season}</td>
                </tr>
              ))}
            </tbody>
          </table>
