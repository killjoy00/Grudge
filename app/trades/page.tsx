        ...
        {players.map((p) => {
          ...
          return (
            <li key={p.player_key} className={counted ? undefined : 'trade-uncounted'}>
              <span className="trade-pos">{POSITIONS[p.default_position_id ?? 0] ?? '—'}</span>
    ...
    <article className="card trade-card">
      <header className="trade-head">
        <span className="eyebrow">
          ...
          {trade.accepted_at && ` · ${new Date(trade.accepted_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`}
        </span>
        {trade.confidence === 'reciprocal' && <span className="tag era" title="Reconstructed from reciprocal weekly roster movement because the completed ESPN item list did not survive.">Reconstructed</span>}
        {trade.confidence === 'manual' && <span className="tag era" title="Entered by hand from the league's own records.">From the record</span>}
        {ungraded ? (
        ...
        {production?.graded && (productionWinner
      ...
      {production?.gradingReason === 'incomplete_data' && <p className="note">Player value is ungraded because some acquired-player scoring or position evidence is missing.</p>}
  ...
  return (
    <div className="card">
      ...
      <div className="scroll" style={{ marginTop: 12 }}>
        <table>
          <thead><tr><th>Manager</th><th>Trades</th><th>W–L</th><th>{valueLabels[0]}</th><th>{valueLabels[1]}</th><th>Net</th></tr></thead>
          <tbody>{rows.map((r) => (
            <tr key={r.franchiseKey}>
              <td>{r.name}</td><td>{r.trades}</td><td>{r.won}–{r.lost}{r.even ? `–${r.even}` : ''}</td>
              <td>{r.gained.toFixed(1)}</td><td>{r.given.toFixed(1)}</td>
...
export default async function Trades({ searchParams }: { searchParams: Promise<{ season?: string }> }) {
  ...
  return (
    <>
      <div className="page-hero compact-hero">
        ...
        <h1>Trades</h1>
        <p>Every trade scored two ways: what it did for the roster, and how good the players themselves were.</p>
      </div>

      {season === current && <p className="note trade-pending-note">A trade appears here once ESPN actually moves the players. An accepted deal sits in ESPN&rsquo;s review window first, and this page picks it up on the following Tuesday &mdash; so a trade agreed this week shows up next week.</p>}

      {cards.length === 0 ? (
        <div className="callout">No trades in {season} yet. Click below for the seasons that have them.</div>
      ) : cards.map((card) => (
      ...
      {(records.length > 0 || productionRecords.length > 0) && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(390px, 1fr))', gap: 14 }}>
            <TradeLedger title="Team fit · lineup impact" note="Best-lineup points gained minus the best-lineup points gained by your trade partners. This rewards filling actual roster holes." rows={records} valueLabels={['Gained', 'Given']} />
            <TradeLedger title="Player value · above replacement" note="Actual acquired-player production above positional replacement, independent of whether your roster happened to have an even better option." rows={productionRecords} valueLabels={['Received', 'Sent']} />
          </div>
        </>
      )}
      ...
      <div className="card">
        <details>
          ...
          <div className="note trade-explainer">
            <p><strong>Team Fit</strong> asks what the deal did to your particular roster. Each week after the trade we take your real roster, work out the best lineup it could have fielded, then do the same for the roster you would have had without the trade. The difference is the trade&rsquo;s lineup impact. It deliberately does not give full credit for a fourth great running back who never improves the lineup.</p>
            <p><strong>Player Value</strong> asks the complementary question: how good were the players you acquired, regardless of your existing roster? Every week you still own an acquired QB, RB, WR or TE, his actual fantasy points are compared with the replacement baseline at his position. Those values are summed and compared with what the other side received.</p>
            <p><strong>Why keep both?</strong> They measure real but different things. Team Fit rewards roster construction and opportunity cost. Player Value grades the talent return without letting the rest of the roster obscure it. A smart trade can win one, both, or neither.</p>
            <p><strong>Dropped and re-traded players.</strong> Both models count a player only while the team that acquired him still owns him. Kickers and defences are listed as part of the historical trade but excluded from both grades.</p>
            <p><strong>Voting</strong> opens on the Tuesday after the week a trade takes effect and runs for seven days. This season only. Your pick stays private until you make it, then you see the league&rsquo;s.</p>
            <p><strong>Where these came from.</strong> For 2018–2025, completed ESPN transaction envelopes are the source of truth whenever the archive preserves the players sent in both directions. From 2026 onward, the same transaction evidence is captured by the live ESPN pipeline. Some accepted historical trades survive only as an empty transaction shell; those are marked <span className="tag era">Reconstructed</span> and are included only when consecutive weekly rosters show players moving both directions between the same two teams. A one-way roster change is never called a trade. The 2005–2017 archives preserve draft-to-final ownership changes, but no reliable trade packages or dates, so those remain research leads and are never graded.</p>
          </div>
        </details>
      </div>

      <SeasonPicker seasons={seasons} current={season} basePath="/trades" heading="Seasons with trades" note="2018 is as far back as the recoverable weekly roster data goes; 2026 onward is captured live." />
    </>
  );
}
