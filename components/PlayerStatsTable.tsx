import { displayNumber, playerHref, playerFilterHref, PLAYER_STATS, statColumns, type PlayerRow, type PlayerFilters } from '../lib/player-data.ts';
  ...
  return <div className="scroll player-table" role="region" aria-label="Player statistics" tabIndex={0}>
    <table><thead><tr>
      ...
      <td className="num player-points"><strong>{displayNumber(p.points, 2)}</strong>
        <span className="tsub block">{p.unavailable ? 'Incomplete' : p.rebuilt ? 'Rebuilt' : p.observed ? 'ESPN' : 'No scores'}</span></td>
